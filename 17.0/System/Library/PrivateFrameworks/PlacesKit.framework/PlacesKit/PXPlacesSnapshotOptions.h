@class PKExtendedTraitCollection, NSString, NSObject;
@protocol PXPlacesMapGeotaggableInfoDelegate, OS_dispatch_queue, PXPlacesMapThumbnailCurationDelegate;

@interface PXPlacesSnapshotOptions : NSObject

@property (retain, nonatomic) PKExtendedTraitCollection *extendedTraitCollection;
@property (retain, nonatomic) id<PXPlacesMapGeotaggableInfoDelegate> geotaggableInformationDelegate;
@property (retain, nonatomic) NSObject<OS_dispatch_queue> *queue;
@property (retain, nonatomic) NSString *cacheDirectoryPath;
@property (nonatomic) double visibleDistance;
@property (nonatomic) unsigned long long snapshotMapType;
@property (nonatomic) long long popoverImageType;
@property (nonatomic) struct CGSize { double width; double height; } viewSize;
@property (nonatomic) BOOL shouldSkipPlaceholder;
@property (nonatomic) BOOL showsPointLabels;
@property (nonatomic) BOOL shouldRenderThumbnailForNoLocation;
@property (nonatomic) struct UIEdgeInsets { double top; double left; double bottom; double right; } additionalInsets;
@property (nonatomic) struct { struct CLLocationCoordinate2D { double latitude; double longitude; } center; struct { double latitudeDelta; double longitudeDelta; } span; } region;
@property (weak, nonatomic) id<PXPlacesMapThumbnailCurationDelegate> mapThumbnailCurationDelegate;

- (id)init;
- (id)description;
- (void).cxx_destruct;

@end
