...
@interface AR3DSkeletonRegistrationTechnique : ARTechnique {
    ...
    struct array<arkit::btr::RegistrationData, 5UL> { struct RegistrationData { struct Matrix<float, 2U, 16U, false> { float m_data[32]; } image_points; struct Matrix<float, 3U, 16U, false> { float m_data[48]; } lifted_points; struct Matrix<float, 3U, 3U, false> { float m_data[9]; } intrinsics; struct SE3GroupStorage<float, cva::Matrix<float, 4, 4>> { float m_data[16]; } cam_from_obj; struct SE3GroupStorage<float, cva::Matrix<float, 4, 4>> { float m_data[16]; } cam_from_vio; double timestamp_obj; double timestamp_vio; } __elems_[5]; } _last_btr_poses;
