@class FigRegToolboxGPU, FigMetalContext, NSDictionary, BWPixelBufferPool, BWAdaptiveCorrectionPreviewRegistrationTuningParameters;
...
- (int)_computeOISCalibrationFromWiderMetadata:(id)a0 narrowerMetadata:(id)a1 calibrationOut:(struct CalModel { double x0[2]; double x1[2]; double x2[2]; double x3[12]; } *)a2;
- (struct { void /* unknown type, empty encoding */ x0[3]; })_homographyAtDepth:(double)a0 calibration:(struct CalModel { double x0[2]; double x1[2]; double x2[2]; double x3[12]; })a1 narrowerMetadata:(id)a2 widerMetadata:(id)a3 pixelBufferDimensions:(struct { int x0; int x1; })a4;
- (int)_updateCorrectedCalibrationUsingInitialOISCalibration:(struct CalModel { double x0[2]; double x1[2]; double x2[2]; double x3[12]; })a0 macroTransitionType:(int)a1;
- (struct { void /* unknown type, empty encoding */ x0[3]; })_fundamentalMatrixFromCalibration:(struct CalModel { double x0[2]; double x1[2]; double x2[2]; double x3[12]; })a0 narrowerMetadata:(id)a1 widerMetadata:(id)a2;
- (int)_estimateDepthFromKeypointsUsingCalibration:(struct CalModel { double x0[2]; double x1[2]; double x2[2]; double x3[12]; })a0 narrowerRawSensorResolution:(struct CGSize { double x0; double x1; })a1 pixelSizeNarrowerInMillimeters:(double)a2 pixelSizeWiderInMillimeters:(double)a3 fundamentalMatrix:(struct { void /* unknown type, empty encoding */ x0[3]; })a4 macroTransitionType:(int)a5 totalZoomFactor:(float)a6 consolidatedDepthsOut:(double *)a7;
