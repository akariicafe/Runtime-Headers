...
@property (readonly, nonatomic) struct tmat4x4<float> { struct tvec4<float> { union { float x; float r; float s; } ; union { float y; float g; float t; } ; union { float z; float b; float p; } ; union { float w; float a; float q; } ; } value[4]; } combined;

+ (id)projectorWithProjection:(const void *)a0 transform:(const void *)a1;
...
- (id)initWithProjection:(const void *)a0 transform:(const void *)a1;
