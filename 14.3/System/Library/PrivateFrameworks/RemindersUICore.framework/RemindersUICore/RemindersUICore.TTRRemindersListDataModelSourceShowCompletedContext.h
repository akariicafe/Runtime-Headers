@interface RemindersUICore.TTRRemindersListDataModelSourceShowCompletedContext : _TtCs12_SwiftObject {
    void /* unknown type, empty encoding */ delegate;
    void /* unknown type, empty encoding */ key;
    void /* unknown type, empty encoding */ showCompletedSnapshot;
    void /* unknown type, empty encoding */ whitelistCompletedReminderIDs;
    void /* unknown type, empty encoding */ userDefaultsObserveToken;
    void /* unknown type, empty encoding */ isSettingShowsCompletedUserDefaults;
}

@end
