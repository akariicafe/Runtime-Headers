@class NSMutableDictionary, MFMessageChangeManager_iOS, EDSearchableIndexScheduler, MFWeakObjectCache, EDMessageQueryParser, NSObject, MFPersistenceDatabase_iOS, EDPersistence, MFFileCompressionQueue, NSString, EDPersistenceHookRegistry, MFLibrarySearchableIndex, MFMessageBodyMigrator, NSMutableSet, _MFMailMessageLibraryStatistics, NSCache;
@protocol OS_os_log, EFScheduler, EFSQLValueExpressable, OS_dispatch_queue;
...
- (void)notifyConversation:(long long)a0 hasMergedIntoConversation:(long long)a1;
...
- (id)copyMessageInfosMatchingCriterion:(id)a0;
...
- (id)copyMessageInfosForConversationsContainingMessagesMatchingCriterion:(id)a0 forMailbox:(id)a1;
- (long long)oldestKnownConversationInMailbox:(id)a0;
- (id)oldestKnownMessageInMailbox:(id)a0;
- (id)sendersByLibraryIDForConversation:(long long)a0 mailbox:(id)a1 limit:(long long)a2;
...
- (void)recomputeUnreadCountForMailboxWithURL:(id)a0;
...
- (id)missingReferencesForConversationContainingMessage:(id)a0;
- (long long)countOfMessagesMissingFromThreadContainingMessage:(id)a0;
...
- (struct sqlite3_stmt { } *)_prepareBatchStatement:(id)a0 pattern:(id)a1 objects:(id *)a2 count:(unsigned long long)a3;
- (struct sqlite3_stmt { } *)_prepareBatchStatement:(id)a0 pattern:(id)a1 libraryIDs:(long long *)a2 batchSize:(unsigned long long)a3;
...
- (id)firstMessageMatchingCriterion:(id)a0;
