...
@interface GEOSnappedRoutePath : NSObject {
    ...
    struct unique_ptr<geo::RouteMapMatchingSection, std::__1::default_delete<geo::RouteMapMatchingSection> > { struct __compressed_pair<geo::RouteMapMatchingSection *, std::__1::default_delete<geo::RouteMapMatchingSection> > { struct RouteMapMatchingSection *__value_; } __ptr_; } _mapMatchingSection;
    BOOL _hasCompletedMapMatching;
    unsigned int _unsnappedPointCount;
    struct { float x0; float x1; float x2; } *_unsnappedPoints;
}
...
- (float)_calculateEdgeLengthFrom:(const struct Matrix<float, 2, 1> { float x0[2]; } *)a0 to:(const struct Matrix<float, 2, 1> { float x0[2]; } *)a1 startCoord:(const struct PolylineCoordinate { unsigned int x0; float x1; } *)a2 endCoord:(const struct PolylineCoordinate { unsigned int x0; float x1; } *)a3;
