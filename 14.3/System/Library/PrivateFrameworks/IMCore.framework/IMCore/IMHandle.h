@class MKMapItem, NSDate, NSDictionary, IMAccount, NSMutableArray, NSString, CNContact, NSSet, NSAttributedString, IMServiceImpl, NSData, NSArray, NSNumber;

@interface IMHandle : NSObject <NSSecureCoding> {
    NSString *_countryCode;
    NSDate *_idleSince;
    NSDictionary *_certs;
    NSSet *_groups;
    NSString *_cnFirstName;
    NSString *_cnLastName;
    NSString *_cnFullName;
    NSString *_cnNickname;
    NSString *_displayID;
    NSString *_firstName;
    NSString *_lastName;
    NSString *_fullName;
    NSString *_nickname;
    NSArray *_emails;
    NSString *_normalizedFormOfID;
    NSString *_IDWithoutResource;
    NSString *_IDWithTrimmedServer;
    NSString *_statusMsg;
    NSString *_processedStatusMsg;
    NSDate *_whenWentOffline;
    NSDate *_whenStatusChanged;
    NSMutableArray *_notificationNameQueue;
    NSMutableArray *_notificationQueue;
    struct __CFPhoneNumber { } *_phoneNumberRef;
    NSString *_formattedNumber;
    unsigned long long _status;
    unsigned long long _capabilities;
    unsigned long long _resourceIndex;
    BOOL _blockNotifications;
    BOOL _hasTemporaryWatch;
    BOOL _isAnonymous;
    BOOL _beingTornDown;
    BOOL _hasCheckedPhoneNumber;
    int _addressBookIdentifier;
    int _notificationQueueCount;
    NSNumber *_isBusiness;
    NSNumber *_isMako;
    NSNumber *_isApple;
}

@property (class, readonly) BOOL supportsSecureCoding;

@property (retain, nonatomic) CNContact *cnContact;
@property (retain, nonatomic) NSDictionary *otherServiceIDs;
@property (nonatomic) long long priority;
@property (nonatomic) long long IDStatus;
@property (retain, nonatomic) MKMapItem *mapItem;
@property (retain, nonatomic) NSData *mapItemImageData;
@property (retain, nonatomic) NSData *mapItemBannerImageData;
@property (copy, nonatomic) NSString *suggestedName;
@property (nonatomic) BOOL hasCheckedForSuggestions;
@property (nonatomic) BOOL hasSuggestedName;
@property (readonly, nonatomic) BOOL hasLocation;
@property (readonly, retain, nonatomic) NSString *_displayNameWithAbbreviation;
@property (retain, nonatomic) NSString *cachedDisplayNameWithAbbreviation;
@property (readonly, retain, nonatomic) NSString *ID;
@property (readonly, retain, nonatomic) NSString *originalID;
@property (readonly, retain, nonatomic) NSString *countryCode;
@property (retain, nonatomic) NSString *personCentricID;
@property (readonly, retain, nonatomic) NSString *displayID;
@property (readonly, retain, nonatomic) NSString *normalizedID;
@property (readonly, nonatomic) BOOL hasName;
@property (readonly, retain, nonatomic) NSString *name;
@property (readonly, retain, nonatomic) NSString *fullName;
@property (readonly, retain, nonatomic) NSString *uniqueName;
@property (readonly, retain, nonatomic) NSString *nameAndID;
@property (readonly, retain, nonatomic) NSString *nameAndEmail;
@property (readonly, retain, nonatomic) NSString *firstName;
@property (readonly, retain, nonatomic) NSString *lastName;
@property (readonly, retain, nonatomic) NSString *nickname;
@property (readonly, retain, nonatomic) NSString *phoneticFirstName;
@property (readonly, retain, nonatomic) NSString *phoneticLastName;
@property (readonly, retain, nonatomic) NSString *phoneticFullName;
@property (readonly, retain, nonatomic) IMAccount *account;
@property (readonly, retain, nonatomic) IMServiceImpl *service;
@property (readonly, nonatomic) unsigned long long status;
@property (readonly, retain, nonatomic) NSString *statusMessage;
@property (readonly, retain, nonatomic) NSString *nameOfStatus;
@property (readonly, retain, nonatomic) NSAttributedString *richStatusMessage;
@property (readonly, retain, nonatomic) NSString *offlineString;
@property (readonly, nonatomic) unsigned long long previousStatus;
@property (readonly, retain, nonatomic) NSString *previousStatusMessage;
@property (readonly, nonatomic) double timeSinceWentOffline;
@property (readonly, nonatomic) double timeSinceStatusChanged;
@property (readonly, nonatomic) unsigned int authRequestStatus;
@property (readonly, nonatomic) double idleTime;
@property (readonly, retain, nonatomic) NSString *idleString;
@property (readonly, retain, nonatomic) NSDate *feedUpdatedDate;
@property (readonly, nonatomic) BOOL canBeAdded;
@property (readonly, nonatomic) BOOL canBeDeleted;
@property (readonly, nonatomic) unsigned long long capabilities;
@property (readonly, nonatomic) BOOL isBot;
@property (readonly, nonatomic) BOOL isBuddy;
@property (readonly, nonatomic) BOOL isLoginIMHandle;
@property (readonly, nonatomic) BOOL isVisiblyBlocked;
@property (nonatomic, setter=setBlocked:) BOOL isBlocked;
@property (readonly, nonatomic) BOOL isAnonymous;
@property (readonly, nonatomic) BOOL isSystemUser;
@property (readonly, nonatomic) BOOL isMobile;
@property (readonly, nonatomic) BOOL isLoginIMHandleForAnyAccount;
@property (readonly, retain, nonatomic) NSString *mobileDeviceName;
@property (readonly, nonatomic) BOOL watchingIMHandle;
@property (readonly, retain, nonatomic) NSData *pictureData;
@property (readonly, retain, nonatomic) NSDictionary *extraProperties;
@property (readonly, retain, nonatomic) NSString *email;
@property (readonly, retain, nonatomic) NSArray *emails;
@property (readonly, retain, nonatomic) NSArray *groupsArray;
@property (readonly, retain, nonatomic) NSSet *groups;
@property (readonly, nonatomic) BOOL hasOtherSiblings;
@property (readonly, retain, nonatomic) NSSet *siblings;
@property (readonly, retain, nonatomic) NSArray *siblingsArray;
@property (readonly, retain, nonatomic) NSArray *accountSiblingsArray;
@property (readonly, retain, nonatomic) NSArray *existingAccountSiblingsArray;
@property (readonly, nonatomic) id bestSibling;
@property (readonly, retain, nonatomic) id bestAccountSibling;
@property (readonly, nonatomic) BOOL isConferenceAvailable;
@property (readonly, nonatomic) BOOL hasConferencing;
@property (readonly, nonatomic) BOOL hasVideo;
@property (readonly, nonatomic) BOOL hasMultiwayVideo;
@property (readonly, nonatomic) BOOL hasAudio;
@property (readonly, nonatomic) BOOL hasMultiwayAudio;
@property (readonly, retain, nonatomic) NSString *resource;
@property (readonly, retain, nonatomic) NSString *IDWithoutResource;
@property (readonly, retain, nonatomic) NSString *server;
@property (readonly, retain, nonatomic) NSString *accountTypeName;
@property (readonly, retain, nonatomic) NSString *guid;

+ (void)setNotificationsEnabled:(BOOL)a0;
+ (id)handlesForCNContact:(id)a0;
+ (BOOL)notificationsEnabled;
+ (id)imageManager;
+ (id)filterIMHandlesForBestAccountSiblings:(id)a0;
+ (void)_loadStatusNames;
+ (id)cnPhoneticKeys;
+ (id)bestIMHandleInArray:(id)a0;
+ (id)nameOfStatus:(unsigned long long)a0;
+ (id)filterIMHandlesForAccountSiblings:(id)a0 onAccount:(id)a1;
+ (void)bestHandlesForCNContacts:(id)a0 useExtendedAsyncLookup:(BOOL)a1 completion:(id /* block */)a2;
+ (void)handlesForCNContacts:(id)a0 useBestHandle:(BOOL)a1 useExtendedAsyncLookup:(BOOL)a2 completion:(id /* block */)a3;
+ (void)validHandlesForCNContacts:(id)a0 useExtendedAsyncLookup:(BOOL)a1 completion:(id /* block */)a2;
+ (void)bestHandlesForCNContacts:(id)a0 completion:(id /* block */)a1;
+ (void)validHandlesForCNContacts:(id)a0 completion:(id /* block */)a1;

- (void)setImageData:(id)a0;
- (void)setEmail:(id)a0;
- (void)_registerForNotifications;
- (BOOL)hasCapability:(unsigned long long)a0;
- (id)init;
- (BOOL)isApple;
- (BOOL)isContact;
- (id)fmfHandle;
- (void)postNotificationName:(id)a0;
- (void).cxx_destruct;
- (id)copyWithZone:(struct _NSZone { } *)a0;
- (id)_contactID;
- (id)initWithAccount:(id)a0 ID:(id)a1 alreadyCanonical:(BOOL)a2;
- (void)dealloc;
- (BOOL)isMako;
- (long long)compareIDs:(id)a0;
- (void)setFirstName:(id)a0 lastName:(id)a1;
- (unsigned long long)hash;
- (id)initWithCoder:(id)a0;
- (BOOL)isBusiness;
- (long long)compareStatus:(id)a0;
- (id)displayNameForChat:(id)a0;
- (id)cnContactWithKeys:(id)a0;
- (id)fmfSiblingHandles;
- (id)description;
- (BOOL)_setCapabilities:(unsigned long long)a0;
- (void)setAnonymous:(BOOL)a0;
- (void)_sendRemoteLogDumpRequest:(id)a0;
- (void)_postNotificationName:(id)a0 userInfo:(id)a1;
- (void)scheduleSuggestedNameFetchIfNecessary;
- (long long)compareAccountNames:(id)a0;
- (void)setEmails:(id)a0;
- (void)setCapabilities:(unsigned long long)a0;
- (void)_postNotification:(id)a0;
- (BOOL)_allowedByScreenTime;
- (void)propertiesChanged:(id)a0;
- (BOOL)hasServer;
- (struct __CFPhoneNumber { } *)phoneNumberRef;
- (BOOL)matchesIMHandle:(id)a0;
- (void)encodeWithCoder:(id)a0;
- (id)imHandleForOtherAccount:(id)a0;
- (id)publicAPIPropertiesDictionary;
- (id)immediateNameWithNeedsSuggestedNameFetch:(BOOL *)a0 useSuggestedName:(BOOL)a1;
- (void)setCustomPictureData:(id)a0 key:(id)a1;
- (void)beginNotificationQueue;
- (void)releaseNotificationQueue;
- (void)_stopRetainingAccount:(id)a0;
- (void)_updateOriginalID:(id)a0;
- (void)_setOriginalID:(id)a0;
- (void)setHasTemporaryWatch:(BOOL)a0;
- (BOOL)resetCNContact;
- (void)updateCNContact:(id)a0;
- (void)statusChanged:(unsigned long long)a0 message:(id)a1;
- (void)statusChanged:(unsigned long long)a0;
- (BOOL)_isChatSiblingOf:(id)a0;
- (void)_fetchBusinessInfo;
- (void)_setOriginalID:(id)a0 updateSiblings:(BOOL)a1;
- (void)_setCountryCode:(id)a0 updateSiblings:(BOOL)a1;
- (id)existingChatSiblingsArray;
- (id)bestIMHandleForService:(id)a0;
- (BOOL)shouldQueueNotifications;
- (BOOL)areCNContactPropertiesRecent;
- (void)resetCNContactProperties;
- (void)_clearCNContactProperties;
- (id)_displayNameWithContact:(id)a0;
- (BOOL)_hasServiceNameProperties;
- (id)_IDWithTrimmedServer;
- (id)_displayNameWithNicknameIfAvailable;
- (id)_formattedPhoneNumber;
- (BOOL)_hasABName;
- (void)_setBaseFirstName:(id)a0 lastName:(id)a1 fullName:(id)a2;
- (void)setFirstName:(id)a0 lastName:(id)a1 fullName:(id)a2 andUpdateABPerson:(BOOL)a3;
- (void)setEmail:(id)a0 andUpdateABPerson:(BOOL)a1;
- (id)_fallbackCNContactWithAllKeys;
- (void)sendNotificationABPersonChanged;
- (BOOL)hasResource;
- (void)_createPhoneNumberRefIfNeeded;
- (BOOL)isBetterThanIMHandle:(id)a0 nonPhoneNumbersPreferred:(BOOL)a1;
- (id)bestIMHandleForAccount:(id)a0 onService:(id)a1 inGroup:(id)a2 otherThan:(id)a3;
- (BOOL)isBetterThanIMHandle:(id)a0;
- (void)_setExtraProperties:(id)a0;
- (BOOL)_isMyIDInList:(id)a0;
- (id)imHandleWithoutResource;
- (void)setStatus:(unsigned long long)a0 message:(id)a1 richMessage:(id)a2;
- (void)_updateStatusBasedOnAuthRequestStatus;
- (void)_filterStatusMessage;
- (void)_clearStatusMessageURLCache;
- (id)_nameForComparisonPreferFirst:(BOOL)a0;
- (id)chatSiblingsArray;
- (id)_chatSiblingsArray;
- (void)customPictureDataChanged:(id)a0 key:(id)a1;
- (void)_mapItemFetchedWithMapItems:(id)a0 error:(id)a1;
- (void)_fetchMapItemImageDataForMapItem:(id)a0;
- (void)_fetchMapItemBannerImageDataForMapItem:(id)a0;
- (void)_postOnScreenChangedNotificationForProperty:(id)a0;
- (void)_mapItemImageDataFetchedWithResultData:(id)a0 error:(id)a1;
- (void)_mapItemBannerImageDataFetchedWithResultData:(id)a0 error:(id)a1;
- (id)initWithAccount:(id)a0 ID:(id)a1;
- (id)_handleInfo;
- (void)_setOriginalID:(id)a0 countryCode:(id)a1 updateSiblings:(BOOL)a2;
- (void)setUniqueName:(id)a0;
- (void)resetUniqueName;
- (void)setLocalNickname:(id)a0;
- (void)setIsMobile:(BOOL)a0;
- (void)setIsBot:(BOOL)a0;
- (void)requestValueOfProperty:(id)a0;
- (void)setValue:(id)a0 ofExtraProperty:(id)a1;
- (id)bestIMHandleForAccount:(id)a0;
- (unsigned int)sortOrderInGroup:(id)a0;
- (id)existingIMHandleWithoutResource;
- (id)dependentIMHandles;
- (void)setAuthRequestStatus:(unsigned int)a0;
- (void)setPersonStatus:(unsigned long long)a0;
- (void)statusMessageChanged:(id)a0;
- (void)_setIDStatus:(long long)a0;
- (void)setIdleSince:(id)a0;
- (void)setFeedUpdatedDate:(id)a0;
- (long long)compareFirstNames:(id)a0;
- (long long)compareLastNames:(id)a0;
- (long long)compareNormalizedIDs:(id)a0;
- (BOOL)isAccountSiblingOf:(id)a0;
- (BOOL)isSiblingOf:(id)a0;
- (id)_chatSiblings;
- (id)_bestChatSibling;
- (void)_sendAutomationData:(id)a0 properties:(id)a1;
- (void)_sendCommand:(id)a0 properties:(id)a1;
- (void)_imPersonPictureChanged:(id)a0;
- (id)customPictureData;
- (void)setCustomPictureData:(id)a0;
- (void)_sendRemoteLogDumpRequest;

@end
