@class NSDictionary, IMChatRegistry, IMTimingCollection, IMMessageItem, IMOrderingTools, NSMutableArray, NSString, NSMutableSet, NSArray, NSMutableDictionary, IMHandle, NSSet, NSData, IMScheduledUpdater, IMInlineReplyController, IMSendProgress, IMMessage, NSDate, IMAccount, NSNumber, MKMapItem;
...
@interface IMChat : IMItemsController <IMStateLoggable, INSpeakable, IMSendProgressDelegate> {
    ...
    unsigned char _hasPendingMarkRead : 1;
...
- (void)join;
- (void)sendDowngradePingForMessage:(id)a0 manualDowngrade:(BOOL)a1 downgradeChat:(BOOL)a2;
...
- (void)joinExistingTUConversationWithVideoEnabled:(BOOL)a0;
...
- (void)sendDowngradePingForMessage:(id)a0 manualDowngrade:(BOOL)a1;
...
- (BOOL)sendDowngradeNotificationTo:(id)a0;
...
- (void)_handleDeliveredCommand:(id)a0;
- (void)_handleIncomingCommand:(id)a0;
- (void)_delayedInvalidateDowngradeState;
- (BOOL)_handleIncomingItem:(id)a0 updateReplyCounts:(BOOL)a1;
...
- (void)_watchHandleStatusChangedForHandle:(id)a0;
...
- (void)_updateChatItemsAsNotSpamEnumeratingItems:(id /* block */)a0;
...
- (void)_unwatchHandleStatusChangedForHandle:(id)a0;
...
- (void)_launchAppForJoinRequest:(id)a0;
