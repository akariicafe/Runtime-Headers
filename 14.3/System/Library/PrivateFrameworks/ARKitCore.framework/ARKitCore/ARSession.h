...
@interface ARSession : NSObject <ARSensorDelegate, ARTechniqueDelegate, ARWorldTrackingTechniqueObserver> {
    ...
    struct { void /* unknown type, empty encoding */ columns[4]; } _cyclopToCameraTransform;
    ...
    struct os_unfair_lock_s { unsigned int _os_unfair_lock_opaque; } _locationSensorLock;
}
...
@property (nonatomic) struct { void /* unknown type, empty encoding */ x0[4]; } cyclopToCameraTransform;
...
- (void)_updateAnchorsForFrame:(id)a0 resultDatas:(id)a1 context:(id)a2 addedAnchors:(id)a3 updatedAnchors:(id)a4 removedAnchors:(id)a5;
