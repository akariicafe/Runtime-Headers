@class NSString, HDProfile, NSMapTable, NSDate, NSObject, HDXPCAlarm;
@protocol OS_dispatch_queue;

@interface HDRestorableAlarmScheduler : NSObject <HDDiagnosticObject, HDDatabaseProtectedDataObserver, HDHealthDaemonReadyObserver> {
    HDProfile *_profile;
    NSObject<OS_dispatch_queue> *_queue;
    int _significantTimeChangeNotificationToken;
    NSMapTable *_clients;
    BOOL _hasInitializedSystemScheduler;
    BOOL _maintenanceOperationQueued;
    BOOL _needsMaintenanceNotifyAndSchedule;
    BOOL _needsLockStateUpdates;
    BOOL _isDeviceOnWrist;
}

@property (copy, nonatomic) NSDate *_unitTest_currentDate;
@property (readonly, nonatomic) BOOL isRegisteredForOnWristUpdates;
@property (readonly, nonatomic) BOOL isRegisteredForLockStateUpdates;
@property (readonly, nonatomic) HDXPCAlarm *systemScheduler;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;

- (id)initWithProfile:(id)a0;
- (id)_currentDate;
- (void)_stopObservingSignificantTimeChangeNotification;
- (void)_queue_registerForWristStateUpdates:(BOOL)a0;
- (void)removeAlarm:(id)a0;
- (void)addAlarm:(id)a0;
- (void).cxx_destruct;
- (void)_queue_updateProtectedDataObserverStateIfRequired;
- (void)dealloc;
- (BOOL)removeAllEventsWithClientIdentifier:(id)a0 error:(id *)a1;
- (void)_startObservingSignificantTimeChangeNotification;
- (void)daemonReady:(id)a0;
- (BOOL)removeEvents:(id)a0 error:(id *)a1;
- (void)_queue_significantTimeChangeDidOccur;
- (id)diagnosticDescription;
- (void)database:(id)a0 protectedDataDidBecomeAvailable:(BOOL)a1;
- (BOOL)_performWriteTransactionAndFireEventsWithError:(id *)a0 block:(id /* block */)a1;
- (void)checkForDueEventsImmediatelyWithCompletion:(id /* block */)a0;
- (void)_queue_fetchCurrentWristStateWithCompletion:(id /* block */)a0;
- (id)scheduledEventsForClientIdentifier:(id)a0 error:(id *)a1;
- (void)_queue_runMaintenanceNotifyAndScheduleWithReason:(id)a0;
- (void)_queue_registerForLockStateUpdates:(BOOL)a0;
- (void)_queue_setNeedsMaintenanceNotifyAndSchedule:(BOOL)a0;
- (void)_queue_beginReceivingSystemEventsIfNecessary;
- (BOOL)_enumerateAllAlarmEventsWithError:(id *)a0 enumerationHandler:(id /* block */)a1;
- (void)_queue_processDueEventsWithCompletion:(id /* block */)a0;
- (void)_queue_enqueueMaintenanceNotifyAndScheduleWithReason:(id)a0;
- (BOOL)_queue_notifyClientsOfDueEventsAndScheduleNextFireDateWithError:(id *)a0;
- (BOOL)replaceAllScheduledEventsWithClientIdentifier:(id)a0 newEvents:(id)a1 error:(id *)a2;
- (BOOL)scheduleEvents:(id)a0 error:(id *)a1;

@end
