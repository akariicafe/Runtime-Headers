...
@interface NEIKEv2ChildSA : NSObject {
    union { union { struct ccec_full_ctx { struct ccec_cp *cp; unsigned char pad[8]; struct ccec_projective_point { unsigned long long xyz[1]; } point[0]; } dhECPKey256[9]; struct ccec_full_ctx { struct ccec_cp *cp; unsigned char pad[8]; struct ccec_projective_point { unsigned long long xyz[1]; } point[0]; } dhECPKey384[13]; struct ccec_full_ctx { struct ccec_cp *cp; unsigned char pad[8]; struct ccec_projective_point { unsigned long long xyz[1]; } point[0]; } dhECPKey521[19]; } dhECPKey; union { unsigned char dhCurveKey25519[32]; } dhCurveKey; } _dhECKey;
}
...
- (struct ccec_full_ctx { struct ccec_cp *x0; unsigned char x1[8]; struct ccec_projective_point { unsigned long long x0[1]; } x2[0]; } *)dhECPKeyForDesc:(int)a0;
